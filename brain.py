...
import pandas as pd
st.set_page_config("Brain Tumour Detection",page_icon=":Brain:")
st.title("Brain Tumor Detection Using Deep Learning")
st.write("""Brain tumors are one of the most critical health issues, requiring early and accurate detection for effective treatment.
...
st.write("""Deep learning models, particularly convolutional neural networks (CNNs), have shown remarkable success in medical image analysis.
...
st.write("""Brain tumors can be broadly categorized into primary and secondary tumors. Primary brain tumors originate within the brain, 
        whereas secondary (metastatic) tumors spread to the brain from other parts of the body.""")
st.write(""" **Gliomas**: These are the most common type of primary brain tumors, arising from glial cells. 
         ...
         **Pituitary Tumors**: Originating in the pituitary gland, 
         these tumors can affect hormone production and cause various systemic symptoms. - 
         **Schwannomas**: These tumors develop from Schwann cells, which produce the myelin sheath covering nerves. 
         An example is the acoustic neuroma, which affects hearing and balance. - 
...
if st.button('Submit'): 
    if patient_name and age and gender and mri_image is not None: 
        ...
        # Assuming `predicted_class` is an array with the prediction result
        f = predicted_class[0]

# Check the value of 'f' for debugging
        print(f"Predicted class: {f}")

# Correct the comparison for f
        if f == 2:
